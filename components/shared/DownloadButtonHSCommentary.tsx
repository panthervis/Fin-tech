...
const DownloadButtonHSCommentary: React.FunctionComponent<any> = props => {
    ...
    const scriptBody = document.createTextNode(
      `if (hbspt && hbspt.cta) {
			  hbspt.cta.load(5610934, "${hubId}", {});
				document.getElementById("commentaryButton").style.display = "none";
      } else {
    ...
    <>
      <div>
        <span className="hs-cta-wrapper" id={`hs-cta-wrapper-${hubId}`}>
          <span
            className={`hs-cta-node font-bold hs-cta-${hubId}`}
            id={`hs-cta-${hubId}`}
          >
            <a
              className="inline-flex items-center bg-white font-bold text-teal-1 rounded-sm py-1 px-3 md:px-3 md:py-1 text-3xs items-center cursor-pointer text-xs"
      ...
      <a id="commentaryButton" href={props.href} target="_self">
...
export default DownloadButtonHSCommentary
